            ...
            foreach (MirConnection mirConnection in SMain.Envir.Connections)
            {
                if (mirConnection.Player.IsGM)
                {
                    if (this.textBoxAmount.Text.Trim() != string.Empty)
                    {
                        if (regex.IsMatch(this.textBoxAmount.Text.Trim()))
                        {
                            if (int.Parse(this.textBoxAmount.Text.Trim()) >= 1)
                            {
                                ...
                                {
                                    mirConnection.Player.ReceiveChat("The location of the brush monster is not a valid point of the current map.", ChatType.System2);
                                }
                                this.textBoxAmount.Text = "若不指定数量,默认为1.";
                            }
                            else
                            {
                                this.textBoxAmount.Text = "怪物数量不能为零!";
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            mirConnection.Player.ReceiveChat("刷怪位置不是当前地图的有效点.", ChatType.System2);
                        }
                        this.textBoxAmount.Text = "若不指定数量,默认为1.";
                    }
                }
